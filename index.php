...
//echo "<tr><th>Productions</th></tr>";
	...
	foreach ($response_data as $i=>$info) {
		if($i % 2 == 0){
			...
			echo '<a href="product.php?id='.$info->id.'" class="btn btn-primary">'.number_format($info->price_ttc, 2, '.', '').'€ TTC</a></div></div>';
			...
			echo '<h5 class="card-title">'.$info->label;
			echo '</h5><p class="card-text">'.$info->description.'</p>';
			echo '<a href="#" class="btn btn-primary">'.number_format($info->price_ttc, 2, '.', '').'€ TTC</a></div></div>';
			//echo "<h2>$info->price_ttc</h2>";
			//echo "<br />";
			//echo "Description: ".$info->description;
			//echo "<br /> <br /></div></a>";
